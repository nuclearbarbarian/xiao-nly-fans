...
export default function TweetCard({ text, date, link, author, images, quotedTweet, retweetedFrom }: Tweet) {
  ...
  return (
    <article className="rounded-xl border border-white/5 bg-card p-5 transition-colors hover:bg-card-hover">
      ...
      <div className="mb-3 flex items-start gap-3">
        <img src={AVATAR} alt="@xiaowang1984" className="h-10 w-10 rounded-full object-cover" />
      ...
      {quotedTweet && (
        <a href={quotedTweet.link || "#"} target="_blank" rel="noopener noreferrer" className="mb-4 block overflow-hidden rounded-lg border border-white/10 bg-white/5 p-3 transition-colors hover:bg-white/10">
          <span className="mb-1 block text-xs font-medium text-pink">{quotedTweet.author}</span>
          <p className="whitespace-pre-wrap break-words text-sm leading-relaxed text-white/70">{quotedTweet.text}</p>
        </a>
      )}
        ...
        <div className={`mb-4 grid gap-2 ${images.length > 1 ? "grid-cols-2" : "grid-cols-1"}`}>
          {images.map((src, i) => (
            <div key={i} className="relative aspect-video w-full overflow-hidden rounded-xl border border-white/5">
              <Image src={src} alt={`Tweet image ${i + 1}`} fill className="object-cover" sizes="(max-width: 600px) 100vw, 600px" unoptimized />
            </div>
          ))}
        </div>
      )}
      ...
      <div className="flex items-center justify-between border-t border-white/5 pt-3">
        <Tooltip tweetText={text} />
        <a href={link} target="_blank" rel="noopener noreferrer" className="shrink-0 text-xs text-pink transition-colors hover:text-primary">
          View on X &rarr;
        </a>
      </div>
    </article>
